...
export async function buscarPorId(req: Request, res: Response): Promise<void> {
  const { empresaId } = contextoDaRequisicao(req);
  res.json(await pedidoService.buscarPorId(empresaId, idDaRota(req)));
}
...
export async function atualizar(req: Request, res: Response): Promise<void> {
  const { empresaId } = contextoDaRequisicao(req);
  res.json(await pedidoService.atualizar(empresaId, idDaRota(req), req.body));
}

export async function atualizarStatus(req: Request, res: Response): Promise<void> {
  const { empresaId } = contextoDaRequisicao(req);
  res.json(await pedidoService.atualizarStatus(empresaId, idDaRota(req), req.body.status));
}

export async function remover(req: Request, res: Response): Promise<void> {
  const { empresaId } = contextoDaRequisicao(req);
  await pedidoService.remover(empresaId, idDaRota(req));
  res.status(204).send();
}
